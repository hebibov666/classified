import { useSelector,useDispatch } from "react-redux"
import { products } from "@/redux/slices/products"
import { useEffect } from "react"
function Card({title,image,price}){

return(
    <div className="w-full bg-white h-auto pb-[5px] max-[632px]:w-1/1 sm:w-1/1 md:w-1/1 lg:w-1/1 xl:w-1/1 rounded-[7px]">
<div className="flex justify-center h-[120px] p-3 w-full">
<img className="rounded-t-[7px] h-full w-full object-contain" src={image}></img>
</div>
<div className="flex flex-col  pl-2  gap-[5px]">
    <h1 className="text-black text-[21px] font-bold">{price}</h1>
    <p className="text-[18px] h-[50px] w-[90%] overflow-ellipsis overflow-hidden">{title}</p>
    <p className="text-[12px] text-[grey] ">21.09.2023</p>
</div>
    </div>
)
}
export default Card