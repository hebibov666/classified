...
function AdvancedSearch(){
   ...
   const form=useSelector(state=>state.book.formId)
    ...
    return(
        <div className={`flex overflow-scroll noscroll flex-col justify-start z-[10000000]  fixed ${show ? 'left-0' : 'left-[-300%]'}  z-[999] anime top-0 left-0  bg-[#00000063] min-h-full h-[100vh]   w-[100%] max-[900px]:w-full  gap-[10px]`}>
...
<div className="flex searchinput shadow-sm shadow-[#f1f3f7] w-full bg-white justify-between p-[2px] items-center  rounded-[10px]">
    ...
    <span className=" bg-blue-600  flex items-center justify-center w-[80px] p-[5px] rounded-[10px] text-white font-bold">Axtar</span>
</div>
...
{city.map(city=>{
  return <option>{city}</option>
                ...
                <button className="bg-blue-600 outline-none border-0 w-full p-2 rounded-[10px] text-white mt-[20px]">Axtar</button>
            </div>
</div>
</div>
</div>
