...
export const userLogin=createAsyncThunk('user/userLogin',async(userData)=>{
  const request=await axios.post("https://classified-c78k.onrender.com/login",userData)
...
export const getUser=createAsyncThunk('user/getUser',async()=>{
 if(typeof window!=="undefined" && window.localStorage){
  ...
  const request=await axios.get('https://classified-c78k.onrender.com/login', {
    headers: {
...
export const RemovePost=createAsyncThunk('user/RemovePost',async(id)=>{
  const request=await axios.delete(`https://classified-c78k.onrender.com/products/${id}`) 
  const response=request.data
